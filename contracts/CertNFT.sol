...
contract CertNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Edufy", " EduNFT") { }

    struct mintet_certificate {
        uint256 ticket_id;
        uint256 token_id;
    }

    mapping(address => mintet_certificate[]) internal mints;
    uint256 public tickets_counter = 0;
    /** @dev mints of a clients
     * @param user address of a client
     * @return mintet_certificate[] array of structs
     */
    function mintsByUser(address user) public view returns (mintet_certificate[] memory) {
        return mints[user];
    }
    ...
    function safeMint(address to, uint256 ticket_id, string memory uri) public {
        ...
        mints[to].push(mintet_certificate(ticket_id, newItemId));
    }
